...
#include "ngx_stream_ip2region_module.h"

static ngx_int_t ngx_stream_ua_parser_test_full_name(char *name);
static ngx_int_t ngx_stream_ip2region_add_variables(ngx_conf_t *cf);
static void *ngx_stream_ip2region_create_conf(ngx_conf_t *cf);
static void ngx_stream_ip2region_cleanup(void *data);
static char *ngx_stream_ip2region_init(ngx_conf_t *cf,
    ngx_command_t *cmd, void *conf);

static ngx_int_t ngx_stream_ip2region_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_stream_module_t ngx_stream_ip2region_ctx = {
    ngx_stream_ip2region_add_variables,    /* pre configuration */
    NULL,                                  /* post configuration */

    ngx_stream_ip2region_create_conf,      /* create main configuration */
...
static ngx_command_t ngx_stream_ip2region_commands[] = {
    { ngx_string("ip2region_db"),
      NGX_STREAM_MAIN_CONF | NGX_CONF_TAKE12,
      ngx_stream_ip2region_init,
...
ngx_module_t ngx_stream_ip2region_module = {
    NGX_MODULE_V1,
    &ngx_stream_ip2region_ctx,     /* module context */
    ngx_stream_ip2region_commands, /* module directives */
...
static ngx_stream_variable_t ngx_stream_ip2region_vars[] = {
    { ngx_string("ip2region"), NULL,
      ngx_stream_ip2region_variable,
      0, 0, 0 },
...
static char *
ngx_stream_ip2region_init(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_stream_ip2region_conf_t *ip2region_cf;
    ...
    if (ngx_stream_ua_parser_test_full_name(db_name) == NGX_OK) {
        db_path = db_name;
...
static void *
ngx_stream_ip2region_create_conf(ngx_conf_t *cf)
{
    ngx_pool_cleanup_t          *cln;
    ngx_stream_ip2region_conf_t *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_stream_ip2region_conf_t));
    if (conf == NULL) {
        return NULL;
    }
    ...
    cln->handler = ngx_stream_ip2region_cleanup;
    cln->data = conf;
...
static ngx_int_t
ngx_stream_ip2region_add_variables(ngx_conf_t *cf)
{
    ngx_stream_variable_t *var;
    ngx_stream_variable_t *v;

    for (v = ngx_stream_ip2region_vars; v->name.len; v++) {
...
static ngx_int_t
ngx_stream_ip2region_variable(ngx_stream_session_t *s,
    ngx_stream_variable_value_t *v, uintptr_t data)
{
    ngx_stream_ip2region_conf_t *ip2region_conf;
    ...
    ip2region_conf = ngx_stream_get_module_main_conf(s, ngx_stream_ip2region_module);
...
static void
ngx_stream_ip2region_cleanup(void *data)
{
    ngx_stream_ip2region_conf_t *ip2region_conf = data;
...
static ngx_int_t
ngx_stream_ua_parser_test_full_name(char *name)
{
#if (NGX_WIN32)
    u_char c0, c1;
